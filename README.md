...
1. Open the `KegMaster\KegMaster.Core\Features\LogOn\B2CConstants.cs` file.
...
#### [OPTIONAL] Step 6a: Configure the iOS project with your app's return URI
 1. Open the `KegMaster.iOS\info.plist` file in a text editor (opening it in Visual Studio won't work for this step as you need to edit the text)
...
#### [OPTIONAL] Step 6b: Configure the Android project with your app's return URI
 ...
 1. Open the `KegMaster.Droid\Properties\AndroidManifest.xml`
...
The structure of the solution is straightforward. All the application logic and UX reside in KegMaster (portable).
...
KegMaster.Droid requires two extra lines in the `MainActivity.cs` file.
...
KegMaster.iOS only requires one extra line, in AppDelegate.cs.
